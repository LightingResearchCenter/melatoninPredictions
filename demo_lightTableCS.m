...
function demo_lightTableCS()
    ...
    %% DEFINE the MONOCHROMATIC SPECTRA    
        ...
        [filenameSPD, path.data] = uigetfile('*.csv',...
                                             'Select the Spectrum file (has to be 2 column file with 1 header row, comma-delimited, and a wavelength range between 380-780nm!)',...
                                             fullfile(path.dataIn, 'lightTableAnnaMurphyGrid.csv'));
    ...
    %% Compute responses and parameters
        ...
        reaTime = toc;
            ...
            cd(path.mainCode)             
            [ratio(:,1)' CSScaled(:,1)' CS(:,1)'] % debug on command window         
